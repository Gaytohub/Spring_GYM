...
public class TrainerServiceImpl implements TrainerService {
    ...
    public Flux<Trainer> getAllTrainers(int pages)
    {
...
//        trainerDao.save(user4);
        ...
        return Flux.fromIterable(trainerDao.findAll(pageable));
    }

    @Cacheable(value = "findTrainersByClub")
    public Flux<Trainer> findByClub(String club,int pages){
        System.out.println("访问数据库来查询教练信息，不走缓存");
        Pageable pageable = PageRequest.of(pages-1,2);
        return Flux.fromIterable(trainerDao.findByClub(club,pageable));
    }

    @Cacheable(value = "findTrainersByClub")
    public Flux<Trainer> findByName(int pages){
        System.out.println("访问数据库来查询教练信息，不走缓存");
        Pageable pageable = PageRequest.of(pages-1,2);
        return Flux.fromIterable(trainerDao.findAll(pageable));
    }
    ...
    public Flux<Trainer> findByClubAndGender(String club, String gender,int pages){
        System.out.println("访问数据库来查询教练信息，不走缓存");
        Pageable pageable =PageRequest.of(pages-1,2);
        return Flux.fromIterable(trainerDao.findByClubAndGender(club, gender,pageable));
    }

    @Cacheable(value = "findTrainersByGender")
    public Flux<Trainer> findByGender(String gender, int pages){
        System.out.println("访问数据库来查询教练信息，不走缓存");
        Pageable pageable =PageRequest.of(pages-1,2);
        return Flux.fromIterable(trainerDao.findByGender(gender,pageable));
    }

    //用于教练详情的查询，不需要分页
    public Mono<Trainer> findByPhone(Long phone){
        System.out.println("访问数据库来查询教练信息，不走缓存");
        return Mono.justOrEmpty(trainerDao.findByPhone(phone));
    }

    public Mono<Trainer> findByPhoneAndPassword(Long phone, Integer password){
        return Mono.justOrEmpty(trainerDao.findByPhoneAndPassword(phone,password));
    }

    public Mono<Trainer> findByName(String name){
        return Mono.justOrEmpty(trainerDao.findByName(name));
    }

    @Override
    public Flux<UserCourse> getUserCourse(Long phone){
        return Flux.fromIterable(userCourseDao.findByTrainerPhone(phone));
    }
}