...
//            Flux<Club> flux = null;
//            List<Club> tempList = null;
//            tempList = flux.collectList().block();
            Flux<Club> list = null;
            Page<Club> tempPage = null;
    ...
    public ModelAndView selfClub(HttpServletRequest request) {
        if (clubService.tryAcquireSeckill()) {
            ...
            Club c = clubService.findByName(name).block();//严谨的写法是在教练表里加一个所在俱乐部的联系方式或者干脆用俱乐部名字做主键
