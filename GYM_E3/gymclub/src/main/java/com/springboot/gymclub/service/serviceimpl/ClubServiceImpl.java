...
public class ClubServiceImpl implements ClubService {
    ...
    public Page<Club> getAllClubs(int pages)
    {
//        Club club1 = new Club();
...
//        clubDao.save(club2);
        ...
        return clubDao.findAll(pageable);
    }

    @Cacheable(value = "findClubsByPhone")
    @Override
    public Page<Club> findByPhone(Long phone,int pages) {
        System.out.println("访问数据库来查询俱乐部信息，不走缓存");
        Pageable pageable =PageRequest.of(pages-1,1);
        return clubDao.findByPhone(phone,pageable);
    }

    @Cacheable(value = "findClubsByName")
    @Override
    public Page<Club> findByName(String name,int pages) {
        System.out.println("访问数据库来查询俱乐部信息，不走缓存");
        Pageable pageable =PageRequest.of(pages-1,1);
        return clubDao.findByName(name, pageable);
    }

    @Cacheable(value = "findClubsByNameAndPhone")
    @Override
    public Page<Club> findByNameAndPhone(String name, Long phone, int pages) {
        System.out.println("访问数据库来查询俱乐部信息，不走缓存");
        Pageable pageable = PageRequest.of(pages-1,1);
        return clubDao.findByNameAndPhone(name,phone,pageable);
    }
